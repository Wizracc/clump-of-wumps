...
let raw_data_set3 = fs.readFileSync(
  "./set3-lite-en_us/en_us/data/set3-en_us.json"
);
const data1 = JSON.parse(raw_data_set1);
const data2 = JSON.parse(raw_data_set2);
const data3 = JSON.parse(raw_data_set3);
const data12 = data1.concat(data2);
const data = data12.concat(data3);
...
function deck_translate(code) {
  try {
    ...
    for (const card of rawDeck) {
      ...
      cardCode = cardCode.replace("undefined", "MT");
      const raw = { ...data.find((o) => o.cardCode === cardCode) };
      const cardInfo = {
...
function quick_deck(code) {
  ...
  if (data !== null) {
    ...
    var typeEmbed = new Discord.MessageEmbed().setTitle(data[1].title);
...
function cards_sort_cost_name(a, b) {
  if (a.cost < b.cost) {
    return -1;
  } else if (a.cost > b.cost) {
    return 1;
  } else {
    return cards_sort_name(a, b);
  }
  return 0;
}

function cards_sort_name(a, b) {
  ...
  if (deck !== null) {
    ...
    for (region in deck.regions) {
      if (deck.regions[region].length > 0) {
        ...
        let region_cards = deck.regions[region];
        region_cards.sort(cards_sort_cost_name);
        for (card of region_cards) {
          embed1_data.regions[index].cards = embed1_data.regions[
            index
          ].cards.concat(
            `\n${rarity_to_emoji(card.rarity)}` +
...
function find_card(args) {
  ...
  var matches = new Set();
  for (var card of data) {
    if (card.name.toLowerCase().trim() === string) {
      matches.add(card);
...
function region_to_emoji(region) {
  const regions = {
    ...
    targon: "<:icontargon:748257274736279730>",
  };
